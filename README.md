...
1. 생성자(Constructor)를 통해 의존성을 낮출 수 있음 
    ...
    ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/6a224336-903a-4873-bfd9-78c4b144cf43/1d8f651e-0a00-4f86-9ce9-0e8b02bf7822/8430cc78-d48f-487b-b521-d489f0e8cccc.png)
...
# Render Modes
의미 : 컴포넌트가 렌더링되는 방식
    1. Statiic Server-Side Rendering(정적 서버 측 렌더링)
        1. 웹 페이지의 모든 컨텐츠를 서버 측에서 렌더링하여 브라우저에 보내는 방식
        2. 클라이언트 측에서는 서버에서 받은 HTML, CSS 및 JavaScript 파일을 렌더링하고 표시함
        3. 초기 로딩시간을 최소화하고 검색 엔진 최적화를 개선
    2. Interactive Server-Side Rendering(대화형 서버 측 렌더링)
        1. 초기 페이지 로드 시에는 서버 측에서 렌더링되지만, 이후 사용자와의 상호 작용에 따라 일부 컨텐츠를 서버 또는 클라이언트 측에서 동적으로 업데이트하는 방식
        2. 초기 로딩 시간을 최소화하고 동시에 사용자 경험을 개선
    3. Client-Side Rendering(클라이언트측 렌더링 - WebAssembly)
        1. 웹 페이지의 모든 렌더링을 브라우저에서 처리하는 방식
        2. 서버는 클라이언트에 필요한 데이터만 제공하고, 클라이언트에서는 이 데이터를 이용해 UI를 동적으로 렌더링 함
        3. 빠른 페이지 전환과 사용자 경험을 제공할 수 있지만, 초기 로딩 시간은 느릴 수 있음
    4. Automatic Rendering(자동 렌더링)
        1. 렌더링 방식이 자동으로 선택되는 방식
        2. 사용자 경험을 최적화
        3. 개발자가 수동으로 렌더링 방식을 선택할 필요없음
    
    - program.cs
        
        ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/6a224336-903a-4873-bfd9-78c4b144cf43/764b06bf-a073-47a7-84b5-4db374b01c72/c6e71f8f-e4b4-48f5-a931-3cfc17b6af7c.png)
